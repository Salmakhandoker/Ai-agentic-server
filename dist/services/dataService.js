...
class DataService {
    ...
    static async updateUserGoogleId(userId, googleId) {
        if (db_1.isConnectedToMongo) {
            return await models_1.User.findByIdAndUpdate(userId, { googleId }, { new: true });
        }
        const user = localUsers.find(u => u._id.toString() === userId);
        if (user) {
    ...
    static async getListings(filters = {}) {
        if (db_1.isConnectedToMongo) {
            return await models_1.Listing.find(filters);
        }
    ...
    static async getListingsAdvanced(options) {
        if (db_1.isConnectedToMongo) {
            ...
            if (options.rating !== undefined)
                query.ratingAverage = { $gte: options.rating };
            let sortObj = { date: -1 };
            if (options.sort) {
                switch (options.sort) {
                    case 'price_asc':
            ...
            const total = await models_1.Listing.countDocuments(query);
            const listings = await models_1.Listing.find(query)
                .sort(sortObj)
        ...
        if (options.q) {
            ...
            result = result.filter(l => l.title.toLowerCase().includes(qLower) ||
    ...
    static async getListingById(id) {
        if (db_1.isConnectedToMongo) {
            return await models_1.Listing.findById(id);
        }
        const oid = id.toString();
        return localListings.find(l => l._id.toString() === oid) || null;
    }
    static async getRelatedListings(listing, limit = 4) {
        if (db_1.isConnectedToMongo) {
            return await models_1.Listing.find({
    ...
    static async createListing(listingData, userId) {
        if (db_1.isConnectedToMongo) {
            const newListing = new models_1.Listing({
                ...listingData,
    ...
    static async deleteListing(id) {
        if (db_1.isConnectedToMongo) {
            return await models_1.Listing.findByIdAndDelete(id);
        }
        const oid = id.toString();
    ...
    static async addReview(listingId, reviewData) {
        if (db_1.isConnectedToMongo) {
            const listing = await models_1.Listing.findById(listingId);
            if (listing) {
                listing.reviews.push({ ...reviewData, date: new Date() });
                const totalRating = listing.reviews.reduce((acc, rev) => acc + rev.rating, 0);
