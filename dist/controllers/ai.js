...
exports.aiRouter.post('/generate-itinerary', async (req, res) => {
    try {
...
function simulateItinerary(title, focus, pace, days) {
    ...
    const activityFocus = focus in morningActivities
        ? focus
        : 'Adventure';
    const highlights = [
...
exports.aiRouter.post('/recommendations', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const listings = await dataService_1.DataService.getListings({});
        const prompt = `
      ...
      ${JSON.stringify(listings.map((l) => ({ id: l._id, title: l.title, category: l.category, price: l.price, location: l.location, duration: l.duration, difficulty: l.difficulty })))}
...
function simulateRecommendations(profile, listings) {
    ...
    const scored = listings.map(listing => {
        ...
        const matchingInterests = interests.filter((interest) => listing.category.toLowerCase().includes(interest.toLowerCase()) ||
...
exports.aiRouter.post('/chat', async (req, res) => {
    try {
        ...
        let contextListing = null;
        if (activeListingId && (typeof activeListingId === 'string' && activeListingId.length === 24)) {
            contextListing = await dataService_1.DataService.getListingById(activeListingId);
        }
        const contextPrompt = `
      ...
      ${messages.map((m) => `${m.role === 'user' ? 'User' : 'Concierge'}: ${m.content}`).join('\n')}
...
function simulateChatResponse(userMessage, url, listing) {
